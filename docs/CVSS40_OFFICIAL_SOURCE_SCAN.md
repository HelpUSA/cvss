# CVSS v4.0 official source scan

Generated UTC: `2026-07-10T11:11:55.965006+00:00`

This document is a source inventory for article work. It intentionally does not copy the full FIRST documentation.

## Sources

| Source | Status | Title | Text length | Key term hits | Heading count |
|---|---:|---|---:|---|---:|
| `main` | 200 | Common Vulnerability Scoring System | 9093 | Base:10; Threat:9; Environmental:2; Supplemental:2; CVSS-B:5; CVSS-BT:3; CVSS-BE:1; CVSS-BTE:2; consumer:1 | 2 |
| `specification` | 200 | CVSS v4.0 Specification Document | 81342 | Base:78; Threat:44; Environmental:37; Supplemental:29; CVSS-B:13; CVSS-BT:8; CVSS-BE:2; CVSS-BTE:6; Security Requirements:1; Modified Base:11; consumer:23 | 42 |
| `user_guide` | 200 | CVSS v4.0 User Guide | 67434 | Base:64; Threat:37; Environmental:21; Supplemental:13; CVSS-B:31; CVSS-BT:9; CVSS-BE:4; CVSS-BTE:7; Security Requirements:2; consumer:9 | 51 |
| `implementation_guide` | 200 | CVSS v4.0 Consumer Implementation Guide | 65005 | Base:86; Threat:61; Environmental:48; Supplemental:14; CVSS-B:12; CVSS-BT:7; CVSS-BE:1; CVSS-BTE:4; Security Requirements:12; Modified Base:11; consumer:25 | 44 |
| `examples` | 200 | CVSS v4.0 Examples | 143693 | Base:162; Threat:64; Environmental:39; Supplemental:7; CVSS-B:1; CVSS-BT:1; CVSS-BTE:1; Security Requirements:5; Modified Base:3; consumer:1 | 58 |
| `faq` | 200 | CVSS v4.0 Frequently Asked Questions | 59282 | Base:48; Threat:41; Environmental:19; Supplemental:19; CVSS-B:7; CVSS-BT:1; CVSS-BTE:1; Security Requirements:3; Modified Base:3; consumer:35 | 48 |
| `calculator` | 200 | Common Vulnerability Scoring System Version 4.0 Calculator | 7403 | Base:2; Threat:5 | 0 |
| `data_representations` | 200 | Common Vulnerability Scoring System Data Representations | 18139 | Base:45; Threat:9; Environmental:17; Supplemental:2 | 8 |

## Captured headings

### main

URL: `https://www.first.org/cvss/v4.0/`

- Common Vulnerability Scoring System Version 4.0
- Chairs

### specification

URL: `https://www.first.org/cvss/v4.0/specification-document`

- Common Vulnerability Scoring System version 4.0: Specification Document
- Introduction
- Metrics
- Assessment
- Nomenclature
- Base Metrics
- Exploitability Metrics
- Attack Vector (AV)
- Attack Complexity (AC)
- Attack Requirements (AT)
- Privileges Required (PR)
- User Interaction (UI)
- Impact Metrics
- Confidentiality (VC/SC)
- Table 6: Confidentiality Impact to the Vulnerable System (VC)
- Table 7: Confidentiality Impact to the Subsequent System (SC)
- Integrity (VI/SI)
- Table 9: Integrity Impact to the Subsequent System (SI)
- Availability (VA/SA)
- Table 10: Availability Impact to the Vulnerable System (VA)
- Table 11: Availability Impact to the Subsequent System (SA)
- Threat Metrics
- Exploit Maturity (E)
- Environmental Metrics
- Confidentiality, Integrity, and Availability Requirements (CR, IR, AR)
- Modified Base Metrics
- Supplemental Metrics
- Safety (S)
- Automatable (AU)
- Provider Urgency (U)
- Recovery (R)
- Value Density (V)
- Vulnerability Response Effort (RE)
- Qualitative Severity Rating Scale
- Vector String
- CVSS v4.0 Scoring
- New Scoring System Development
- CVSS v4.0 Scoring using MacroVectors and Interpolation
- Scores of all MacroVectors
- Appendix A - Acknowledgments
- Appendix B - On-Line Resources
- Version History

### user_guide

URL: `https://www.first.org/cvss/v4.0/user-guide`

- Common Vulnerability Scoring System version 4.0: User Guide
- Introduction
- Changes in CVSS Version 4.0
- CVSS Nomenclature
- CVSS Base Score (CVSS-B) Measures Severity, not Risk
- Changes to Assessment Guidance
- Scope Removed
- Assessing Vulnerabilities in Software Libraries (and Similar)
- Multiple CVSS Base (CVSS-B) Scores
- Guidance for Using Environmental Security Requirements Metrics
- Guidance for Using Supplemental Metrics
- New Base Metric: Attack Requirements
- Updated Base Metric: User Interaction
- Temporal renamed to the Threat Metric Group
- The CVSS Extensions Framework
- New Scoring System Development
- Use metric groups to gather the 15 million CVSS-BTE vectors into 270 equivalence sets
- Solicit experts to compare vectors representing each equivalence set
- Use the expert comparison data to calculate an order of vectors from least severe to most severe
- Solicit expert opinion to decide what vector group in the ordering of vectors represents the boundary between qualitative severity scores to be backwards compatible with qualitative severity score boundaries from CVSS v3.x.
- Compress the vector groups in each qualitative severity bin into the number of available scores in that bin (for example, 9.0 to 10.0 for critical, 7.0 to 8.9 for high, etc.)
- Create a small score modification factor that adjusts the scores of vectors within a vector group so that a change of any metric value results in a score change. The intent is that the score change is not larger than the uncertainty in the ranking of the vector groups as collected from the expert comparison data in step 2.
- Update to the Version Identifier in the Vector String
- Assessment Guide
- Integrate Vulnerability Scan results with Asset Management
- Integrate Vulnerability Scan results with Threat Intelligence
- CVSS Scoring in the Exploit Life Cycle
- Confidentiality and Integrity, Versus Availability Impacts
- Local Vulnerabilities Exploited by Remote Attackers
- Vulnerability Chaining
- Vulnerable System and Subsequent System
- Vulnerable Systems Protected by a Firewall
- CVSS Extensions Framework
- Guidelines
- Suggested Vector String Format
- Attack Vector Considerations
- Non-Repudiation is a Part of Integrity
- Security Requirements
- Confidentiality Requirement (CR)
- Integrity Requirement (IR)
- Availability Requirement (AR)
- Supplemental Metrics
- Safety
- Automatable
- Provider Urgency
- Recovery
- Value Density
- Vulnerability Response Effort
- Glossary of Terms
- Scoring Rubrics
- Version History

### implementation_guide

URL: `https://www.first.org/cvss/v4.0/implementation-guide`

- Common Vulnerability Scoring System version 4.0 Consumer Implementation Guide
- Introduction
- Executive Summary
- Key Takeaway
- What is not in This Guide
- Aim and Audiences
- CVSS Metric Groups and Lifecycle
- CVSS Vector Strings
- CVSS Vector String Enrichment
- CVSS Threat Metrics
- CVSS Environmental Metrics
- Asset Management Systems and Environmental Metrics
- Modified Exploitability Metrics
- Modified Attack Vector (MAV)
- Modified Attack Complexity (MAC)
- Modified Attack Requirements (MAT)
- Modified Privileges Required (MPR)
- Modified User Interaction (MUI)
- Modified Vulnerable System Impact Metrics
- Modified Vulnerable System Confidentiality (MVC)
- Modified Vulnerable System Integrity (MVI)
- Modified Vulnerable System Availability (MVA)
- CVSS Security Requirements (Environmental) {#cvss-security-requirements-(environmental)}
- CVSS Supplemental Metrics
- CVSS Maturity
- CVSS Maturity Model and Vector String Map
- Principles for CVSS Maturity Levels
- Automatable at Scale
- Difficulty of Obtaining Data
- Cost of Obtaining Data
- Level of Effort to Generate Data
- Levels of Maturity
- CVSS Maturity Level Zero
- CVSS Maturity Level One
- CVSS Maturity Level Two
- CVSS Maturity Level Three
- CVSS Maturity Plus “+”
- Key Take Away
- Conclusion
- Appendix A - Acknowledgements
- Appendix B - Network Architecture Examples
- The Impact of Secure Architecture on CVSS Scores
- Sample Network Configurations and CVSS Score Impacts
- Version History

### examples

URL: `https://www.first.org/cvss/v4.0/examples`

- Common Vulnerability Scoring System v4.0: Examples
- Document Version: 1.8
- Resources & Links
- Introduction
- Common Vulnerability Scoring System version 4.0 Examples
- New metric coverage
- New Metric – Attack Requirements
- CVE-2022-41741
- CVE-2020-3549
- CVE-2023-3089
- Revised Metric – User Interaction
- CVE-2021-44714
- CVE-2022-21830
- New Metric – Subsequent Confidentiality, Availability, Integrity
- CVE-2022-22186
- CVE-2023-21989
- CVE-2020-3947
- CVE-2023-48228
- New Metric – Safety
- CVE-2023-30560
- CISA KEV Examples
- CVE-2026-48172
- Classic Examples
- OpenSSL Heartbleed Vulnerability ( CVE-2014-0160 )
- Apache log4j JNDI Command Execution “log4shell” Vulnerability ( CVE-2021-44228 )
- GNU Bourne-Again Shell (Bash) ‘Shellshock’ Vulnerability ( CVE-2014-6271 )
- Juniper Proxy ARP Denial of Service Vulnerability (CVE-2013-6014)
- Lenovo ThnkPwn Exploit ( CVE-2016-5729 )
- Failure to Lock Flash on Resume from sleep ( CVE-2015-2890 )
- Intel DCI Issue ( CVE-2018-3652 )
- Common Vulnerabilities Classes
- regreSSHion – CVE-2024-6387
- SQL Injection – CVE-2023-30545
- On-path Attacker – CVE-2021-23846
- Denial of Service – CVE-2023-22394
- Cross-Site Scripting (Reflected) – CVE-2022-24682
- Cross-Site Scripting (Stored) – CVE-2020-0926
- Cross-Site Scripting Expanded Impact – CVE-2024-55228
- Cross-Site Request Forgery – CVE-2023-5602
- Privilege Escalation (Unprivileged) CVE-2022-20759
- Privilege Escalation (Highly Privileged) CVE-2021-34724
- Remote Code Execution ( CVE-2023-28311 )
- Arbitrary Code Execution CVE-2022-22965
- Physical Access (CVE-2022-20826)
- Information Disclosure – CVE-2022-21500
- Information Disclosure - CVE-2021-32570
- Command Injection ( CVE-2022-26134 )
- ACL Bypass ( CVE-2023-20245 )
- Server-Side Request Forgery (SSRF) ( CVE-2024-1233 )
- Industrial Control Systems (ICS) ( CVE-2023-28728 )
- Operational Technology (OT) ( CVE-2022-47379 )
- IOT - Healthcare ( CVE-2020-10627 )
- Value Density ( CVE-2020-28196 )
- Management System ( CVE-2023-20048 )
- Ephemeral Build Systems ( CVE-2024-23897 )
- Linux Kernel ( CVE-2026-31431 )
- Windows Kerberos DoS ( CVE-2026-42903 )
- Version History

### faq

URL: `https://www.first.org/cvss/v4.0/faq`

- Common Vulnerability Scoring System v4.0: Frequently Asked Questions (FAQ)
- Introduction
- Resources & Links
- Frequently Asked Questions
- Is there a prescribed way to use CVSS Base and Environmental metrics to score a vulnerability along a long supply chain?
- What’s the difference between Attack Complexity (AC) and Attack Requirements (AT)?
- Why did you use “AT” for Attack Requirements, instead of “AR”?
- What are the boundaries between a Vulnerable System and Subsequent System?
- Supplemental Metrics do not affect the final score. What is the recommended way to consume and respond to Supplemental Metric assessment?
- What is the guidance on what action to take for the various values of the Safety (S) Supplemental Metric: Negligible (N) and Present (P)?
- Additionally, what is meant by “consequences of the vulnerability meet definition of IEC 61508 consequence categories of ‘marginal,’ ‘critical,’ or ‘catastrophic.’”?
- What is the guidance on what action to take for the various values of the Automatable (AU) Supplemental Metric: No (N) and Yes (Y)?
- What is the guidance on what action to take for the various values of the Recovery (R) Supplemental Metric: Automatic (A), User (U), and Irrecoverable (I)?
- What is the guidance on what action to take for the various values of the Value Density (V) Supplemental Metric: Diffuse (D) and Concentrated (C)?
- What is the guidance on what action to take for the various values of the Vulnerability Response Effort (RE) Supplemental Metric: Low (L), Moderate (M), and High (H)?
- What is the guidance on what action to take for the various values of the Provider Urgency (U) Supplemental Metric: Clear, Green, Amber, and Red?
- Additionally, why were TLP colors chosen, rather than simply Low, Moderate, and High?
- How did CVSS v4.0 address the issue of scores being too clustered toward High and Critical severity ratings?
- How does the value of Attack Complexity (AC) or Attack Requirements (AT) vary with the introduction of compensating controls (e.g., WAF, CSP, etc.)?
- Which value of User Interaction (UI), Passive (P) or Active (A), should be used for Reflected Cross-Site Scripting (XSS) or Stored Cross-Site Scripting vulnerabilities?
- Is clicking a link Passive (P) or Active (A) User Interaction?
- Which system is considered the Vulnerable System or the Subsequent System when assessing Reflected or Stored Cross-Site Scripting (XSS) vulnerabilities?
- Which system is considered the Vulnerable System or the Subsequent System when assessing SQL Injection vulnerabilities?
- “It is the responsibility of the consumer to populate the Threat Metric Exploit Maturity (E) based on information regarding the availability of exploitation code/processes and the state of exploitation techniques.”
- Why is this the responsibility of the consumer and not the provider (vendor)?
- Why does the application of Threat data only reduce the resulting CVSS Score?
- How does a consumer apply threat intelligence data into a CVSS Assessment?
- How does a consumer apply Environmental data into a CVSS Assessment?
- Is EPSS or SSVC a replacement for CVSS?
- How should the Attack Requirements metric be used with regards to system configuration?
- How is the User Interaction metric used in relation to CSRF vulnerabilities?
- Why are CVSS v4.0 scores different from version v3.1 scores?
- Scores for the same vulnerability are different between v3.1 and v4.0
- Why do some unique CVSS v4.0 metrics result in the same numeric score?
- Why do some metrics, such as User Interaction or Privileges Required, have less than expected impact on the resulting score?
- Why do subsequent system metric scores with any combination of non- High SC, SI, or SA metrics result in the same score?
- How can I implement a CVSS calculator for my own organization?
- Calculator Validation Steps
- List of Calculator Libraries
- How can I apply CVSS concepts to AI / LLM application issues?
- What are best practices for vulnerability assessments that differ depending on the platform?
- How should threats be assessed using the Exploit Maturity metric?
- How can CVSS v3.1 vectors be derived from v4.0 vectors?
- Does the use of CVSS require a CVE ID or Record?
- How does AIVSS relate to CVSS?
- Why does one vulnerability have different CVSS scores?
- How does CVSS provide input to patch priority?
- Version History

### calculator

URL: `https://www.first.org/cvss/calculator/4.0`

_No headings captured._

### data_representations

URL: `https://www.first.org/cvss/data-representations`

- Common Vulnerability Scoring System Data Representations
- Explanation of Schemas
- JSON
- Previous Standard Versions JSON Formats
- JSON Examples
- XML
- Previous Standard Versions XML Formats
- XML Examples
