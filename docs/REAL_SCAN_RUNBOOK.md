...
### CI boundary for evidence exports

The GitHub workflow intentionally runs `python scripts/validate_real_pipeline.py` without `--export-evidence`. Evidence bundles under `outputs/evidence/` are local review artifacts and should not be generated by the default CI gate.

## Evidence export manifest contract

When `python scripts/validate_real_pipeline.py --export-evidence` or `python scripts/export_real_evidence.py` is used, the generated `outputs/evidence/latest/manifest.json` is expected to preserve this review contract:

- `schema` is `helpus.cvss.real_evidence_export` and `schema_version` is `1`.
- `generated_at` is an ISO-8601 UTC timestamp.
- `finding_count` matches the assessment and dashboard summaries, and remains zero unless `--allow-findings` is explicitly used for investigation evidence.
- `files` maps the copied scan, assessment, dashboard baseline, and optional assessment report inside the exported bundle.
- `source_paths` records the original scan, assessment, and dashboard baseline input paths.
- `scan_summary` and `assessment_summary` carry the source review metadata needed to audit the bundle without reopening every input file.
