from core.cvss31 import base_score, official_cvss_output, parse_vector
...
def test_parse_vector_requires_base_metrics():
 try: parse_vector('CVSS:3.1/AV:N/AC:L')
 except ValueError as exc: assert 'missing required base metrics' in str(exc)
 else: raise AssertionError('expected missing metrics error')

def test_official_output_is_separate():
