...
def calculate(row):
    ...
    if b(row.get("internet_exposed", False)):
        add(
            ...
            "increase",
            0.4,
            "internet exposure increased environmental risk",
        )
    else:
        add(
            "internet_exposed",
            "decrease",
            -0.3,
            "absence of internet exposure reduced environmental risk",
        )
    ...
    if b(row.get("network_segmented", False)):
        add(
            ...
            "decrease",
            -0.7,
            "network segmentation reduced reachable attack surface",
        )
    ...
    if b(row.get("firewall_restricted", False)):
        add(
            ...
            "decrease",
            -0.5,
            "firewall restrictions reduced exposure",
        )
    ...
    if b(row.get("compensating_controls", False)):
        add(
            ...
            "decrease",
            -0.4,
            "compensating controls reduced expected impact",
        )
    ...
    if b(row.get("pci_in_scope", False)):
        add(
            ...
            "increase",
            0.3,
            "PCI scope increased business and compliance relevance",
        )
    ...
    if c == "high":
        add(
            ...
            "increase",
            0.4,
            "high business criticality increased environmental relevance",
        )
    elif c == "low":
        add(
            "business_criticality",
            "decrease",
            -0.2,
            "low business criticality reduced environmental relevance",
        )
    ...
    return {
        ...
        "environmental_severity": sev(env),
        "delta": delta,
        "decision": decision,
        "rationale": rationale,
        "adjustment_trace": trace,
        "trace_count": len(trace),
        "trace_total_adjustment": round(sum(t["amount"] for t in trace), 2),
    }
