\section{Preliminary Evaluation Plan and Baseline Results}
\label{sec:evaluation}

The current artifact includes a clean deterministic validation run for the PCI segmented lab case. The run is stored in \path{outputs/runs/pci_segmented_lab_20260517_143146/} and is accompanied by \path{run_manifest.json}, input hashes, generated assessments, an audit trace, and a before-after comparison table. These results are treated as artifact-validation results, not as evidence that the watcher workflow outperforms human experts.
