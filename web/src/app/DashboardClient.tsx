...
import { CuratedRunsPanel, type Run as CuratedRun } from "./CuratedRunsPanel";
...
export function DashboardClient({ data }: { data: any }) {
  ...
  const curatedRuns: CuratedRun[] = data.curatedRuns ?? [];
  const manifest = data.manifest as any;
  const realAssessment = data.realAssessment as any;
  ...
  return (
    <main className="page">
      ...
      {realAssessment ? (
        <section className="panel">
          <h2>Real Trivy baseline</h2>
          <p>Latest automated local repository scan generated by tools/run_real_assessment.py.</p>
          <div>
            Findings: <strong>{realAssessment.summary?.finding_count ?? 0}</strong>
          </div>
          <div>
            Critical: {realAssessment.summary?.critical ?? 0} · High: {realAssessment.summary?.high ?? 0} · Medium: {realAssessment.summary?.medium ?? 0} · Low: {realAssessment.summary?.low ?? 0}
          </div>
          <div>
            Asset: <span className="mono">{realAssessment.asset?.id ?? "unknown"}</span>
          </div>
          <div>
            Scanner: <span className="mono">{realAssessment.scanner ?? "trivy"}</span>
          </div>
        </section>
      ) : null}
